...
#Checking how many missing values are there in each variable
colSums(is.na(diabeticData))

# Removing column with more than 40% missing values
diabeticData$weight <- NULL
diabeticData$payer_code <- NULL
diabeticData$medical_specialty <- NULL

#creation of some dummy variables since some variables had many categories
# Race dummy
...
# Aggregating the age variable into 3 categories instead of 9: Young, middle aged, and old
...
#Creating dummy variables for different treatments patients had
...
# The output variable has 3 classes: "<30", No (no readmission) and ">30" (readmitted after 30 days).
# Since We are only interested in the admission within 30 days,
# we aggregate the two classes "No" and ">30" together.
...
#Some patients' data were not taken once, which influences independence between variables.
# We will keep only one (the first) observation per patient.
#This leaves us with approximately 70.000 observation
